...
@ba_rd_split_router.get("/tpl_voucher_entry", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.post("/imp_voucher_entry", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/gen_voucher_entry", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/tag_voucher_entry", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.post("/get_voucher_entry", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/tpl_product_cost", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.post("/imp_product_cost", tags=["3.3.1 研发分配核算"])
async def imp_product_cost(file: UploadFile = File(...)):
    ...
    try:
        if file.content_type in ("application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"):
            ...
            ba_rd_split_service.fill_code_product()
            return ok()
        else:
            return fail(415, "文件类型错误，请上传Excel xlsx 或 xls格式")
    except sqlite3.OperationalError as oe:
...
@ba_rd_split_router.get("/tpl_para_special_retrospect", tags=["3.3.1 研发分配核算"])
async def tpl_para_special_retrospect():
    """
    获取序时账验证时特殊追溯人工设置表模板
    """
    try:
        file_for_download = "para_special_retrospect.xlsx"
        excel_content = db.export_through_mem("tpl_para_special_retrospect")
        response = Response(content=excel_content, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response.headers['Content-Disposition'] = 'attachment; filename=' + file_for_download
        return response
    except sqlite3.OperationalError as oe:
...
@ba_rd_split_router.post("/imp_para_special_retrospect", tags=["3.3.1 研发分配核算"])
async def imp_para_special_retrospect(file: UploadFile = File(...)):
    """
    导入序时账验证时特殊追溯人工设置表
    """
    try:
        if file.content_type in ("application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"):
            await db.import_through_mem(file, "para_special_retrospect", False)
            return ok()
        else:
            return fail(415, "文件类型错误，请上传Excel xlsx 或 xls格式")
    except sqlite3.OperationalError as oe:
...
@ba_rd_split_router.get("/tpl_para_ba_rd_split_ratio", tags=["3.3.1 研发分配核算"])
async def tpl_para_ba_rd_split_ratio():
    """
    获取研发分配核算系数人工设置表模板
    """
    try:
        file_for_download = "para_ba_rd_split_ratio.xlsx"
        excel_content = db.export_through_mem("tpl_para_ba_rd_split_ratio")
        response = Response(content=excel_content, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response.headers['Content-Disposition'] = 'attachment; filename=' + file_for_download
        return response
    except sqlite3.OperationalError as oe:
...
@ba_rd_split_router.post("/imp_para_ba_rd_split_ratio", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/tpl_para_cost_account_transfer", tags=["3.3.1 研发分配核算"])
async def tpl_para_cost_account_transfer():
    """
    获取需跳过（报制单类）或结转等特殊处理的成本科目人工设置表模板
    """
    try:
        file_for_download = "para_cost_account_transfer.xlsx"
        excel_content = db.export_through_mem("tpl_para_cost_account_transfer")
        response = Response(content=excel_content, media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        response.headers['Content-Disposition'] = 'attachment; filename=' + file_for_download
        return response
    except sqlite3.OperationalError as oe:
...
@ba_rd_split_router.post("/imp_para_cost_account_transfer", tags=["3.3.1 研发分配核算"])
async def imp_para_cost_account_transfer(file: UploadFile = File(...)):
    """
    导入需跳过（报制单类）或结转等特殊处理的成本科目人工设置表
    """
    try:
        if file.content_type in ("application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"):
            await db.import_through_mem(file, "para_cost_account_transfer", False)
            return ok()
        else:
            return fail(415, "文件类型错误，请上传Excel xlsx 或 xls格式")
    except sqlite3.OperationalError as oe:
...
@ba_rd_split_router.get("/tpl_para_caccount_raccount", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.post("/imp_para_caccount_raccount", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/tpl_para_project_capitalized_cost", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.post("/imp_para_project_capitalized_cost", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/tpl_para_recycle", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.post("/imp_para_recycle", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/process_map_ccenter_caccount_svoucher", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/get_map_ccenter_caccount_svoucher", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.post("/get_voucher_detail", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.post("/select_voucher", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/split_voucher", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.post("/get_voucher_splitted", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/get_stat_raccount", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/get_stat_project", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/get_stat_caccount", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/get_stat_after_adjusted_left", tags=["3.3.1 研发分配核算"])
...
@ba_rd_split_router.get("/get_stat_after_adjusted_right", tags=["3.3.1 研发分配核算"])
